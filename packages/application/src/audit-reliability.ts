import { logger, type AuditKind } from '@app/domain';
...
export async function safeAudit(
  ...
  try {
    ...
    logger.error(`[audit] write failed (kind=${kind}); recording dead-letter`, { error: message });
    try {
      await recordDeadLetter(payload, message);
    } catch (dlqErr) {
      logger.error('[audit] dead-letter write also failed', { error: dlqErr instanceof Error ? dlqErr.message : dlqErr });
    }
  }
}
