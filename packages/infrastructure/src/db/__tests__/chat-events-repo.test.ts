...
let insertCalls = 0;
  ...
  it('updateEventMeta merges via jsonb || and reports matched rows', async () => {
    const { client, executed } = makeExecuteClient([{ id: 7 }]);
    const result = await new ChatEventBatcher(client).updateEventMeta('turn-1', { notice: true });
  ...
  it('updateEventMeta is false for an empty patch or missing turnId without querying', async () => {
    ...
    await expect(batcher.updateEventMeta('', { notice: true })).resolves.toBe(false);
    expect(executed).toHaveLength(0);
  });

  it('getQualitySamples filters by the blocked column with random order', async () => {
    const orderBys: SQL[] = [];
    const wheres: SQL[] = [];
    const fullRow = {
      id: 1,
  ...
  it('getJudgeAverages maps averages with coalesce-to-zero semantics', async () => {
    const { client, executed } = makeExecuteClient([
      { avg_faithfulness: '0.8666666666666667', avg_retrieval_relevance: null },
    ]);
    ...
    expect(query).toContain("-> 'judgescores' ->> 'retrievalrelevance'");
    expect(query).not.toContain('degraded');
  });
  ...
  it('getDailyQuality groups judge aggregates per day', async () => {
    ...
    expect(byDay.get('2026-08-22')).toEqual({ day: '2026-08-22', avgFaithfulness: 0.9, avgRetrievalRelevance: 0.4 });
    expect(byDay.get('2026-08-23')).toEqual({ day: '2026-08-23', avgFaithfulness: 0, avgRetrievalRelevance: 0 });
    expect(byDay.get('2026-08-21')).toEqual({ day: '2026-08-21', avgFaithfulness: 0, avgRetrievalRelevance: 0 });
    const query = compiled(executed);
  ...
  it('patchMeta merges into a buffered unflushed event and no-ops otherwise', async () => {
    ...
    expect(
      batcher.patchMeta('11111111-1111-4111-8111-111111111111', { notice: true, fallbackReason: 'turn_deadline' }),
    ).toBe(true);
    expect(batcher.patchMeta('22222222-2222-4222-8222-222222222222', { notice: true })).toBe(false);
    await batcher.flush();
...
suite('ChatEventBatcher purge, anonymize & metrics (real SQL)', () => {
  ...
  it('updateEventMeta merges the patch while preserving existing keys', async () => {
    try {
      await db.transaction(async (tx) => {
        ...
        await expect(batcher.updateEventMeta(uuid(99), { notice: true })).resolves.toBe(false);
        throw ROLLBACK;
      });
    } catch (e) {
      expect(e).toBe(ROLLBACK);
  ...
  it('getQualitySamples honors the blocked filter with random order', async () => {
    try {
      await db.transaction(async (tx) => {
        const batcher = new ChatEventBatcher(tx);
        await tx.insert(chatEvents).values([
          { userId: 'qs-u', mode: 'agentic', meta: { notice: true } },
          { userId: 'qs-u', mode: 'vector', hallucinationBlocked: true },
          { userId: 'qs-u', mode: 'vector', meta: {} },
        ]);
        const all = await batcher.getQualitySamples(10, {});
  ...
  it('getQualitySamples excludes rows older than 7 days from the filtered variant', async () => {
    try {
      await db.transaction(async (tx) => {
        const batcher = new ChatEventBatcher(tx);
        const stale = new Date(Date.now() - 8 * 86_400_000);
        await tx.insert(chatEvents).values([
          { userId: 'qs-stale', mode: 'agentic', hallucinationBlocked: true, createdAt: stale },
  ...
  it('getJudgeAverages returns zeros on an empty window', async () => {
    try {
      await db.transaction(async (tx) => {
        const batcher = new ChatEventBatcher(tx);
        const averages = await batcher.getJudgeAverages(7);
        expect(averages).toEqual({ avgFaithfulness: 0, avgRetrievalRelevance: 0 });
        throw ROLLBACK;
      });
    } catch (e) {
      expect(e).toBe(ROLLBACK);
