...
function warnEnvFallback(key: string, raw: string | undefined, fallback: number | string): void {
  if (raw !== undefined) logger.warn(`[config] Invalid ${key}="${raw}" — using default "${fallback}"`);
}

function positiveIntegerOrDefault(value: string | undefined, fallback: number, key?: string): number {
  const n = finiteOrDefault(value, fallback);
  const ok = Number.isInteger(n) && n > 0;
  if (!ok && key) warnEnvFallback(key, value, fallback);
  return ok ? n : fallback;
}

function nonnegativeIntegerOrDefault(value: string | undefined, fallback: number, key?: string): number {
  const n = finiteOrDefault(value, fallback);
  const ok = Number.isInteger(n) && n >= 0;
  if (!ok && key) warnEnvFallback(key, value, fallback);
  return ok ? n : fallback;
...
function positiveOrDefault(value: string | undefined, fallback: number, key?: string): number {
  const n = finiteOrDefault(value, fallback);
  const ok = n > 0;
  if (!ok && key) warnEnvFallback(key, value, fallback);
  return ok ? n : fallback;
...
function nonnegativeOrDefault(value: string | undefined, fallback: number, key?: string): number {
  const n = finiteOrDefault(value, fallback);
  const ok = n >= 0;
  if (!ok && key) warnEnvFallback(key, value, fallback);
  return ok ? n : fallback;
...
function probabilityOrDefault(value: string | undefined, fallback: number, key?: string): number {
  const n = finiteOrDefault(value, fallback);
  const ok = n >= 0 && n <= 1;
  if (!ok && key) warnEnvFallback(key, value, fallback);
  return ok ? n : fallback;
...
function enumOrDefault<T extends string>(value: string | undefined, allowed: readonly T[], fallback: T, key?: string): T {
  const found = allowed.find((candidate) => candidate === value) ?? fallback;
  if (value !== undefined && found === fallback && value !== fallback) logger.warn(`[config] Invalid ${key}="${value}" — using default "${fallback}"`);
  return found;
}
...
function resolveRuntimeConfig(env: EnvSource) {
  const rawLogLevel = env.get('LOG_LEVEL');
  const LOG_LEVEL: LogLevel = VALID_LOG_LEVELS.includes(rawLogLevel as LogLevel)
    ? (rawLogLevel as LogLevel)
    : (() => {
        if (rawLogLevel !== undefined) logger.warn(`[config] Invalid LOG_LEVEL="${rawLogLevel}" — using default "info"`);
        return 'info' as LogLevel;
      })();

  const BLOB_GET_MAX_BYTES = positiveIntegerOrDefault(env.get('BLOB_GET_MAX_BYTES'), DEFAULT_BLOB_GET_MAX_BYTES, 'BLOB_GET_MAX_BYTES');
  const UPLOAD_CHUNKED_MAX_MD_BYTES = positiveIntegerOrDefault(env.get('UPLOAD_CHUNKED_MAX_MD_BYTES'), DEFAULT_UPLOAD_CHUNKED_MAX_MD_BYTES, 'UPLOAD_CHUNKED_MAX_MD_BYTES');
  const UPLOAD_CHUNKED_MAX_PDF_BYTES = positiveIntegerOrDefault(env.get('UPLOAD_CHUNKED_MAX_PDF_BYTES'), DEFAULT_UPLOAD_CHUNKED_MAX_PDF_BYTES, 'UPLOAD_CHUNKED_MAX_PDF_BYTES');
  const PDF_PARSE_MAX_BYTES = positiveIntegerOrDefault(env.get('PDF_PARSE_MAX_BYTES'), DEFAULT_PDF_PARSE_MAX_BYTES, 'PDF_PARSE_MAX_BYTES');
  const PDF_PARSE_MAX_PAGES = positiveIntegerOrDefault(env.get('PDF_PARSE_MAX_PAGES'), DEFAULT_PDF_PARSE_MAX_PAGES, 'PDF_PARSE_MAX_PAGES');
  const PDF_PARSE_MAX_CHARS = positiveIntegerOrDefault(env.get('PDF_PARSE_MAX_CHARS'), DEFAULT_PDF_PARSE_MAX_CHARS, 'PDF_PARSE_MAX_CHARS');
  ...
  const MD_CHUNK_DELIMITER =
    ...
    !/[\r\n]/.test(configuredDelimiter)
      ? configuredDelimiter
      : (() => {
          if (configuredDelimiter !== undefined) logger.warn(`[config] Invalid MD_CHUNK_DELIMITER="${configuredDelimiter}" — using default`);
          return DEFAULT_MD_CHUNK_DELIMITER;
        })();
  ...
  const INGEST_CHUNK_SIZE = positiveIntegerOrDefault(env.get('INGEST_CHUNK_SIZE'), DEFAULT_INGEST_CHUNK_SIZE, 'INGEST_CHUNK_SIZE');
  ...
  const EVAL_FAITHFULNESS_THRESHOLD = probabilityOrDefault(env.get('EVAL_FAITHFULNESS_THRESHOLD'), DEFAULT_EVAL_FAITHFULNESS_THRESHOLD, 'EVAL_FAITHFULNESS_THRESHOLD');
