...
export const fieldConfig: Record<string, FieldMeta> = {
  ...
  'agentStepBudget':      { group: 'Retrieval', label: 'Agentic Step Budget', inputType: 'slider', min: 1, max: 20, step: 1,
                            helpText: 'Caps LLM tool steps and retry passes.' },
  ...
  'rerankerThreshold':    { group: 'Retrieval', label: 'Reranker Threshold', inputType: 'slider', min: 0, max: 1, step: 0.05,
                            helpText: 'Independent relevance floor applied only after a successful reranker pass.' },
  ...
  'lexicalSearchMode':    { group: 'Retrieval', label: 'Lexical Search Mode', inputType: 'select',
                            helpText: 'Weighted web search uses title/section boosts and web-style phrase, OR, and exclusion syntax. Content plain is the rollback path.' },
  'rerankerProvider':     { group: 'Retrieval', label: 'Reranker Provider', inputType: 'select' },
  'auxModel':             { group: 'Retrieval', label: 'Aux Model Override', inputType: 'text',
                            helpText: 'Model id for query rewriting, hallucination checking, and sampled quality judges.' },
  'judgeSampleRate':      { group: 'Retrieval', label: 'Judge Sample Rate', inputType: 'slider', min: 0, max: 1, step: 0.01,
                            helpText: 'Fraction of answered turns scored by sampled quality judges (analytics only).' },
  ...
  'parentChunkSize':      { group: 'Chunking', label: 'Parent Chunk Size', inputType: 'number', min: 200, max: 10000,
                            helpText: 'Parent-child strategy only: context block size. Applies to new uploads only.' },
  'childChunkSize':       { group: 'Chunking', label: 'Child Chunk Size', inputType: 'number', min: 100, max: 5000,
                            helpText: 'Retrieval unit size for parent-child children and document-aware chunks (overlap derives as 10%). Applies to new uploads only.' },
  ...
  'rsePenalty':           { group: 'Chunking', label: 'RSE Penalty', inputType: 'slider', min: 0, max: 1, step: 0.05,
                            helpText: 'Segment mode only: relevance subtracted per chunk; chunks below zero count against their segment.' },
  'rseMaxSegmentChunks':  { group: 'Chunking', label: 'RSE Max Segment', inputType: 'slider', min: 1, max: 20, step: 1,
                            helpText: 'Segment mode only: maximum chunks in one reconstructed segment.' },
  'rseOverallMaxChunks':  { group: 'Chunking', label: 'RSE Overall Max', inputType: 'slider', min: 1, max: 30, step: 1,
                            helpText: 'Segment mode only: chunk budget across segments per contiguous run.' },
  'rseMinSegmentValue':   { group: 'Chunking', label: 'RSE Min Value', inputType: 'slider', min: 0, max: 2, step: 0.05,
                            helpText: 'Segment mode only: minimum segment value required for emission.' },
