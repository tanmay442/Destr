...
orchestrator cutover, and all test/eval migrations are done and green.
Every claim here was verified against current code; corrections to the
workstream brief are called out inline.

## Removed names and their replacement path

| Removed | Replacement |
|---|---|
| `packages/application/src/rag/agentic-search.ts` (rewrite → hybrid/rerank retrieve → retry-once wrapper) + `export * from './agentic-search'` in `packages/application/src/rag/index.ts` | `runStructuredSearch` in `packages/application/src/agent/search/search-orchestrator.ts`, wired as `structuredSearch` in `src/composition.ts:401-408` and passed straight through in `src/app/api/chat/handler.ts` (conditional-spread probing removed) |
| `comp.agenticSearch` + `getAgenticDeps` in `src/composition.ts` (incl. the inline `AGENTIC_ENABLED=false` normal-direct fallback inside the wrapper) | `comp.structuredSearch` (always wired); the `AGENTIC_ENABLED=false` rollback now lives once in the turn seam (`turn.ts:331`) and runtime config (`src/lib/config/runtime.ts:100,182-183`) |
...
| `SUPPORT_AGENT_ENABLED` (`agent/agent-flags.ts` deleted, barrel re-export removed from `agent/index.ts`) | `SupportAgent` loop always runs (`turn.ts:848-850`); the one-step no-tools fallback was deleted, not kept |
...
- First-turn prefetch is bypassed whenever the orchestrator serves the turn
  (`turn.ts:667-669`), so prefetch can never shadow orchestrated evidence.
...
- Rollback after WP-9 is a revertible commit per removed flag; there is no
  ...
  grounding, or the planner experiment flags. `AGENTIC_ENABLED=false`
  (normal-direct retrieval) is the supported production rollback.
...
1. Cache key versions (verified current; versions unchanged, field sets narrowed):
   ...
   so entries keyed under the old field sets will miss, not collide.
   Wire formats: coordination key `rag:turn-result:{user}:{turnId}` plus
   versioned key `rag:turn-result:v2:{user}:{turnId}` (`turn.ts:360-365`),
...
3. Fingerprint removal status: RESOLVED — `legacyTurnRequestFingerprint`
   ...
   removed; `parseTurnResult` accepts exactly `current` + `preResultContract`.
   Pre-v2 entries are a transparent miss (recomputed; the verified-grounding
   check would reject them anyway), never a replay and never a 409 — only a
   v2 fingerprint mismatch on the same turn id is an idempotency conflict.
   `legacySearchResultCacheFingerprint` is DELIBERATELY RETAINED: it feeds the
   `preResultContract` read-compat window for turn-result entries written
   before the result-contract version; removal condition is the TTL/idempotency
   window lapsing plus a second confirmation, tracked as follow-up.
