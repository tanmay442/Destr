...
- **Sweeper** — `POST/GET /api/admin/queue/sweep` (cron `0 3 * * *`, gated by `CRON_SECRET` via `Authorization: Bearer`, timing-safe compare) marks documents stuck `queued` past 24 h as `failed` — a backstop for failures that never reached the DLQ (e.g. DLQ delivery itself failing, or old messages enqueued before DLQ was configured). The route is public at the middleware level but enforces its own `CRON_SECRET`/admin check, mirroring `/api/admin/analytics/rollup`.
